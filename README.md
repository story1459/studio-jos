...
화면 **아무 데서나 통통 튀며 나타나** 곡선을 그리며 헤엄치고, **또 아무 데서나 통통 튀며 사라집니다.**
지나간 자리에는 물파장이 남습니다.

| 무엇 | 범위 |
|---|---|
| 등장 간격 | 3.5 – 9초 (`GAP`) |
| 시작·끝 지점 | 화면 안 임의의 두 점 (서로 충분히 떨어지도록 다시 뽑음) |
| 동선 | 두 점을 잇는 선을 수직으로 밀어 만든 곡선 (길이의 12 – 28%, 방향은 랜덤) |
| 크기 | 화면 폭의 7 – 12%, 56 – 140px 로 제한 |
| 헤엄치는 시간 | 3.8 – 6.4초 |
| 진하기 | 0.65 – 0.9 |

**갈퀴 방향** — `ART_FACES` 로 아트워크가 향한 쪽을 정합니다(현재 `-1` = 왼쪽).
진행 방향과 다르면 좌우를 뒤집어, 갈퀴가 늘 앞을 향합니다.
전부 반대로 돌리고 싶으면 이 값 하나만 뒤집으면 됩니다.

**움직임** — 세 겹으로 나눠 각각 다른 변형을 겁니다.

| 층 | 하는 일 |
|---|---|
| `.shark` | 곡선 동선 이동 + 전체 투명도 |
| `.shark__pop` | 나타날 때 `scale(0 → 1.28 → .92 → 1)`, 사라질 때 `1 → 1.22 → 0` |
| `.shark__bob` | 몸을 ±7° 틀며 위아래로 통통 (1.05초) + 진행 방향 반전 |

**물파장** — 0.3초마다 조스의 현재 위치 아래쪽에 타원 링을 남깁니다.
링은 1.5초에 걸쳐 2.1배로 퍼지며 사라지고, 최대 40개까지만 유지합니다.

크기는 컴포넌트의 `w` 계산식, 등장 빈도는 `GAP`, 물파장 간격은 `RIPPLE_EVERY`,
움직임은 `app/globals.css` 의 `shark-pop` · `shark-bob` · `ripple-out` 에서 고칩니다.
